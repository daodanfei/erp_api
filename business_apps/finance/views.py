...
class FinanceViewSet(viewsets.ViewSet):
    ...
    def dashboard(self, request):
        ...
        trend = FinanceStatsService.get_cash_flow_trend() if policy.cash_flow_analysis_enabled() else []
        return Response({
            'kpis': kpis,
            'trend': trend
        })
    ...
    def aging(self, request):
        ...
        summary = FinanceStatsService.get_aging_summary(aging_type)
    ...
    def customer_reconciliation(self, request, customer_id=None):
        ...
        try:
            statement = ReconciliationService.get_customer_statement(customer_id, start_date, end_date)
    ...
    def supplier_reconciliation(self, request, supplier_id=None):
        ...
        try:
            statement = ReconciliationService.get_supplier_statement(supplier_id, start_date, end_date)
            return Response(statement)
        except Exception as e:
            return Response({"detail": str(e)}, status=status.HTTP_400_BAD_REQUEST)
...
class CashAccountViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if not policy.multi_cash_account_enabled() and CashAccount.objects.filter(status=True).exists():
